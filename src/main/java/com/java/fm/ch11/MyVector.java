...
public class MyVector implements List {
    ...
    public MyVector(int capacity) {
        if (capacity < 0) {
            throw new IllegalArgumentException("유효하지 않은 값임. : " + capacity);
        }
    ...
    public Object get(int index) {
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException("범위를 벗어났음.");
        }
    ...
    public Object remove(int index) {
        ...
        // 마지막 데이터를 null로 함. 배열은 0부터 시작하므로 마지막 요소는 index가 size - 1임.
        data[size - 1] = null;
        size--;
        return oldObj;
    }
